  ...
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <li v-for="author in modernAuthors" :key="author.id">
        {{ author.name }} - born at {{ author.birthYear }}
      </li>
      </ul>
    ...
    <section class="lab-section">
      ...
      <!-- 这个和11不一样，要用for去遍历，因为花括号里面有两个数据"storeTypes": { "physical": 3,"online": 2} -->
      <ul>
      ...
      <!-- 在对象数据当中不需要什么map，flagmap，只需要不停的点点点就ok，一个花括号就是一个点"openingHours": {
    "weekdays": { "open": "9:00 AM", "close": "9:00 PM" },
    "weekends": { "open": "10:00 AM", "close": "10:00 PM" }
  } -->
      <ul>
      <li>
        <strong>Weekdays:</strong>
        {{ bookstores.openingHours.weekdays.open }} - {{ bookstores.openingHours.weekdays.close }}
      </li>
      <li>
        <strong>Weekends:</strong>
        {{ bookstores.openingHours.weekends.open }} - {{ bookstores.openingHours.weekends.close }}
      </li>
      </ul>
      ...
      {{ country }}
      	...
      	<!-- topSellers[0]--访问 topSellers 数组中的第一个元素 -->
    ...
    <section class="lab-section">
      ...
      <ul>
         <!-- author in authors对 authors 数组中的每一项进行遍历，把当前这一项赋值给变量 author -->
          <!-- :class="{ active: isSelected }"	控制是否添加类名 -->
           <!-- style和clas功能一样但是clas用的更多 -->
      <li
      v-for="author in authors"
      :key="author.id"
      :class="{ highlight: author.id === highlightId }"
      >
